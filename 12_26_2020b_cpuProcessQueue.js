...
// process taking the entire CPU and freezing all the other processes. One solution
...
// are the same size then it is First In First Out (FIFO). The idea is that the
// shorter jobs will finish quicker, so theoretically jobs won't get frozen because
...
// It takes in:
//
// "jobs" a non-empty array of positive integers. They represent the clock-cycles(
//   cc) needed to finish the job.
...
function SJF(jobs, index){
  let minutes = 0;
  let myVal = jobs[index];
  for(var i = 0; i < jobs.length; i++){
    if(jobs[i] < myVal){
      minutes += jobs[i];
    }
    else if(jobs[i] == myVal && i <= index){
      minutes += jobs[i];
    }
  }
  return minutes;
...
let SJF = (jobs, index) => jobs.filter((eachItem, i) => eachItem < jobs[index] || (eachItem == jobs[index] && i <= index)).reduce((accum, eachEle) => accum + eachEle, 0);
